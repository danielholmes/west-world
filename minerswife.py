...
class MinersWife(GameEntity):
    ...
    def stew_finished(self):
        self.sing_out("Stew's ready, let's eat!")
        self._is_cooking = False
        self._messages.dispatch(sender=self, receiver_id=1, message_type=MessageType.STEW_READY)
...
class CookStewState(GameEntityState):
    def enter(self, wife):
        if not wife.is_cooking:
            wife.sing_out("Hi honey, let me make you some of mah stew")
            wife.start_stew()
